...
| 前置条件 | 用户已进入某个空间 |
| 相关模块 | 文档树、中心编辑区、SQLite 持久化、搜索、导出与分享 |
...
第一版提供独立 Spreadsheet 文档类型。用户从新建菜单创建“新建 Excel”后，目录树出现“无标题表格”，点击后中心区进入 Spreadsheet 编辑界面。第一版聚焦可编辑、可保存、可重开，不承诺完整 Excel 兼容、多人协作、云同步、复杂权限、完整导出和打印。
...
4. 第一版导出与分享不默认支持 Spreadsheet，避免生成错误交付物。
...
| Spreadsheet 编辑态 | 中心区显示在线 Excel 风格表格 | 编辑单元格、自动保存、收藏、重命名 |
| 保存中 | 用户编辑后等待自动保存 | 继续编辑 |
| 保存失败 | 本地写入失败 | 保留当前编辑状态，提示保存失败 |
| 不支持导出分享 | Spreadsheet 文档暂不展示 Markdown/Word/PDF/分享操作 | 等待后续能力 |
...
| 自动保存中 | 状态 | 正在保存 |
| 自动保存失败 | 错误 | 保存失败 |
| 导出暂不支持 | 禁用提示 | Spreadsheet 暂不支持该导出方式 |
...
7. Spreadsheet 文档不展示或禁用当前不支持的 Markdown、Word、PDF 和分享操作。
8. 数据库 migration 有回归测试覆盖。

## 15. 待确认问题

本功能当前无阻塞待确认问题。第一版默认使用“新建 Excel”作为菜单文案，默认标题为“无标题表格”，`.xlsx` 导入导出作为后续增强。
