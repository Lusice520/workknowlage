...
1. 先扩展关联文档 view model，补充推荐理由和证据强度。
2. 再更新右侧栏 UI，提供全部线索详情、推荐理由和轻反馈入口。
3. 轻反馈优先做成本地会话级降噪，不先引入复杂持久化。
4. 大模型关系判定先只补类型和候选 payload 边界，不在 P0 中直接接入模型调用。
5. 图谱候选写入需要等待大模型配置、数据边界和关系类型枚举确认。
