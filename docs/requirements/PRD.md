...
| v1.1 | Codex | 将分享能力从单一局域网分享升级为分享下拉：局域分享 + 临时公网分享；明确公网分享需要 Cloudflare Tunnel、自动密码和关闭时间 | 2026-05-20 |
| v1.2 | Codex | 新增外部 Markdown 文件打开能力：独立窗口、左侧目录、自动保存回原文件和手动导入知识库 | 2026-05-20 |
| v1.3 | Codex | 优化分享体验：新增分享启动中状态、当前会话内重复复制公网链接和密码、共享链接管理视图和一键关闭所有链接 | 2026-05-21 |
...
| 分享访问 | 局域分享与临时公网分享访问成功率 | 待测 | ≥ 95% | 真实浏览器验证 |
| 分享管理 | 已开启链接可发现、可复制、可关闭 | 待测 | 100% 当前空间活跃分享可见 | focused tests + 手工验证 |
...
WorkKnowlage 的目标是把“记录、组织、查找、关联、导出、分享、复盘”收敛到一个 local-first 工作流中。用户可以在本地空间中创建和维护资料，通过搜索和关系导航找回上下文，并在需要时导出、局域分享或临时公网分享。
...
当用户需要把资料交付给他人时，可以选择导出 Markdown、Word、PDF，或通过分享下拉选择局域分享、临时公网分享。导出流程应生成可打开、内容顺序正确、结构尽量保真的文件；局域分享生成 LAN IP 地址，让同一局域网内其他设备访问只读页面；临时公网分享通过 Cloudflare Tunnel 生成公网临时链接，并自动生成访问密码和关闭时间。分享启动过程可能持续数秒，标题状态胶囊必须展示启动中状态。已开启的分享必须能在“共享链接”集合视图中被集中查看、复制、关闭或一键关闭。该流程的核心要求是：交付结果可用、状态可感知、错误可解释、分享关闭或到期后链接失效。
...
flowchart LR
    ...
    Choose -->|分享下拉| ShareMode{"选择分享模式"}
    ShareMode -->|局域分享| LocalShare["启动局域网只读分享"]
    LocalShare --> Lan["生成 LAN IP 分享地址"]
    Lan --> LocalReadonly["同局域网访问只读分享页"]
    ShareMode -->|临时公网分享| PublicShare["设置关闭时间并启动 Cloudflare Tunnel"]
    PublicShare --> Password["系统自动生成访问密码"]
    Password --> PublicUrl["生成公网临时链接"]
    PublicUrl --> PublicReadonly["输入密码后访问只读分享页"]
    Md --> Deliver["交付文件或链接"]
    Docx --> Deliver
    Pdf --> Deliver
    LocalReadonly --> Close["用户关闭分享"]
    PublicReadonly --> Close
    Close --> Expired["旧链接失效"]
```
...
flowchart TB
    ...
    ExportShare --> ShareServer["分享服务\nLAN IP / Cloudflare Tunnel / 只读页面"]
    UI --> Release["发布交付\nmacOS DMG"]
```
...
| macOS 桌面端 | 文档编辑器 | 外部 Markdown 文件 | Finder 默认打开、独立外部文件窗口、自动保存回原文件、导入知识库 | P0 | 让 WorkKnowlage 可作为外部 Markdown 文件编辑器 |
| macOS 桌面端 | 文档编辑器 | 提醒块 | warning、error、info、success | P0 | 风险、提示、说明信息 |
| macOS 桌面端 | 文档编辑器 | 富表格 | 插入、编辑、粘贴、键盘、工具栏、导出 | P0 | 结构化内容记录 |
| macOS 桌面端 | 文档编辑器 | Spreadsheet 文档类型 | 新建 Excel、在线表格编辑、本地自动保存 | P0 | 独立结构化数据资料 |
...
| macOS 桌面端 | 关系导航 | Wiki 关联推荐解释 | 推荐理由、全部线索、轻反馈、智能判定预留 | P0 | 保持右侧 Wiki 的推荐辅助定位 |
...
| macOS 桌面端 | 导出与分享 | 分享下拉 | 局域分享、临时公网分享、模式状态、复制与关闭 | P0 | 按场景选择分享方式 |
| macOS 桌面端 | 导出与分享 | 局域分享 | LAN IP、只读页面、复制、刷新、关闭 | P0 | 同局域网临时查看 |
| macOS 桌面端 | 导出与分享 | 临时公网分享 | Cloudflare Tunnel、公网临时链接、自动访问密码、关闭时间、到期失效 | P0 | 非同局域网临时查看 |
| macOS 桌面端 | 导出与分享 | 共享链接视图 | 当前空间活跃分享列表、复制、关闭、重置公网链接和一键关闭所有链接 | P0 | 集中管理已开放链接 |
...
| 本地工作区 | 文档树 | P0 | 已实现 | `docs/requirements/specs/document_tree_spec.md` | 创建、移动、展开收起、选中态、空状态、根目录拖拽投放和手动排序持久化 |
...
| 文档编辑器 | 外部 Markdown 文件打开 | P0 | 已实现 implemented | `docs/requirements/specs/external_markdown_file_spec.md` | Finder 默认打开 `.md` / `.markdown`，独立外部文件窗口，左侧目录，自动保存回原文件，手动导入知识库 |
...
| 文档编辑器 | Spreadsheet 文档类型 | P0 | 已建 ready-for-implementation | `docs/requirements/specs/spreadsheet_document_spec.md` | 独立表格文档类型；新建 Excel、在线表格编辑、本地 workbook 保存 |
...
| 关系导航 | Wiki 关联推荐与智能判定 | P0 / P1 | 已建 draft | `docs/requirements/specs/wiki_association_intelligent_judgement_spec.md` | 右侧推荐解释、全部线索、低打扰反馈；大模型关系判定作为图谱候选基础 |
...
| 导出与分享 | 分享下拉与临时分享 | P0 | 已实现，待真实公网与 DMG 验证 | `docs/requirements/specs/local_share_spec.md` | 覆盖局域分享、临时公网分享、Cloudflare Tunnel、自动访问密码、关闭时间、只读分享页、状态、异常、内置 `cloudflared` 和布局验收 |
...
除了知识库内部文档，用户也需要把 WorkKnowlage 作为外部 Markdown 文件编辑器使用。系统应支持 macOS 将 `.md` / `.markdown` 默认打开到 WorkKnowlage，并在独立窗口中显示外部文件，而不是把它混入当前知识库目录树。外部文件窗口应保留 WorkKnowlage 块编辑体验，左侧显示文档目录，顶部显示文件名、路径、自动保存状态、修改时间和字数；右上角提供“在 Finder 中显示”和“导入知识库”。外部文件默认自动保存回原始 Markdown 路径，只有用户主动导入时才创建内部知识库文档。

在基础编辑之外，WorkKnowlage 需要支持更适合项目资料和产品文档的结构化内容。提醒块用于标记风险、提示、错误和成功信息；富表格用于记录任务清单、问题列表、设备对比、需求优先级等嵌入式结构化资料；Spreadsheet 文档类型用于承载类似在线 Excel 的独立表格资料，例如预算、排期、验收矩阵、数据对比和项目台账；图片和附件用于保存截图、参考资料和交付物。富表格是高复杂度能力，需要支持插入、编辑、粘贴、键盘操作、工具栏和导出，同时不能破坏外层编辑器的光标、选择和块结构。Spreadsheet 文档类型应与普通文档共享目录树、移动、删除、重命名和收藏能力，但内容存储应独立于 BlockNote `content_json`，避免污染普通文档解析和导出链路。
...
关联线索被发现之后，右侧 Wiki 的核心职责是推荐和解释，而不是要求用户逐条审核。系统应支持用户查看某个关联文档的全部线索，理解推荐理由，并通过低打扰反馈优化后续排序。用户反馈可以帮助降噪，但不应成为建立知识关系的必经流程，也不应把右侧栏变成待处理任务列表。

如果产品目标是形成有效关联图谱，关系确认的主路径应由大模型智能判定承担。系统可以先用确定性规则召回候选，再将候选文档、证据片段和上下文交给大模型判断关系是否成立、属于什么类型、置信度如何。高置信判定结果进入图谱候选层；低置信结果仅作为右侧推荐参考。所有智能判定都应保留原始证据、推荐理由、置信度和模型来源，不能只写入不可解释的关系边。
...
### 8.6 分享下拉：局域分享与临时公网分享

用户在某些场景下需要临时把当前文档发给其他设备或同事查看。分享入口应保留为当前文档的分享下拉，并在下拉中提供两种明确模式：局域分享和临时公网分享。局域分享保留现有 LAN IP 能力，适合同一局域网临时阅读；临时公网分享通过 Cloudflare Tunnel 生成公网临时链接，适合发给不在同一局域网的人。用户点击分享后，不能只看到按钮置灰，标题状态胶囊应立刻进入“正在开启 / 正在生成公网链接 / 正在关闭”等中间态。

局域分享不能只把链接显示成 `127.0.0.1`，因为这只对本机有效；share server 也不能只监听 loopback 地址，应监听非 loopback 地址，例如 `0.0.0.0`。临时公网分享必须内置访问密码，密码由系统每次自动生成，不要求用户手动输入；创建时必须设置关闭时间，支持到期自动失效和手动关闭。公网密码只在创建或重置时返回明文，持久化存储只保存哈希；当前 App 会话可以临时记住密码并支持重复复制，重启后如需再次获得密码，应通过“重置公网链接和密码”生成新链接和新密码。关闭或到期后，系统应禁用分享 token，并停止对应公网 tunnel，使旧链接不可继续访问。分享页应展示标题、时间、正文、提醒块、列表、表格和目录，适合阅读但不允许编辑；宽屏布局要让正文和目录形成整体居中的阅读组，避免目录孤立在右侧导致视觉重心偏斜。

左侧快捷入口应提供“共享链接”，用于查看当前空间内所有正在开放的局域分享和临时公网分享。该视图应支持复制局域链接、复制公网链接、在当前会话内复制公网链接和密码、重置公网链接和密码、关闭单个局域 / 公网分享，以及一键关闭当前空间所有分享。
...
7. 用户可以查看 Wiki 关联文档的全部证据线索；右侧 Wiki 不要求用户逐条手动确认关系。
8. Markdown、Word、PDF 导出能处理常见文档。
9. Word 导出不会因颜色 token 报错。
10. 分享下拉保留局域分享，并新增临时公网分享，两种模式入口清晰。
11. 局域分享可被同一局域网设备访问。
12. 临时公网分享需要访问密码和关闭时间，关闭或到期后旧链接失效。
13. 分享页宽屏阅读布局协调。
14. 分享启动、关闭和重置过程中有明确中间态提示。
15. “共享链接”视图能展示当前空间所有活跃分享，并支持复制、关闭、重置公网链接和一键关闭。
14. `npm run typecheck`、focused tests、`npm run build` 和必要 packaging 命令通过。
...
3. 将 Wiki 关联从算法发现推进到推荐解释、低打扰反馈和智能关系判定。
4. 准备 graph view。
