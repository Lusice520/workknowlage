...
| macOS 桌面端 | 关系导航 | Wiki 关联线索确认 | 固定、忽略、转为手动关联、查看全部线索、确认状态持久化 | P0 | 将算法发现转为用户可控的知识关系 |
...
| 关系导航 | Wiki 关联线索确认与固化 | P0 | 已建 draft | `docs/requirements/specs/wiki_association_confirmation_spec.md` | 固定、忽略、转为手动关联、查看全部线索和本地确认状态持久化 |
...
关联线索被发现之后，用户还需要判断、采纳和沉淀这些线索。系统应支持用户对关联文档执行固定、忽略、转为手动关联和查看全部线索。固定用于让有价值但暂未进入正文的关联长期保留；忽略用于减少误命中反复出现；手动关联用于把算法发现固化为用户确认的知识关系；查看全部线索用于替代只靠 hover 预览判断的轻量模式。该能力的产品边界是“用户确认层”：算法负责发现候选，用户动作负责决定哪些关系进入长期知识结构。

用户确认状态应本地持久化，并和当前 workspace、当前文档、目标文档绑定。固定和手动关联应优先展示并计入 Wiki 角标；已忽略的算法关联不应计入角标，也不应在普通关联列表中反复出现。已忽略只影响算法派生的关联文档，不应隐藏用户真实写入的文档提及、出链或反向链接。
...
7. 用户可以对 Wiki 关联文档固定、忽略、转为手动关联，并查看全部证据线索。
