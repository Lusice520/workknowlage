...
import {
  ...
  getActiveFolders,
  getActiveDocuments,
  getTrashedFolderRoots,
  getTrashedDocumentRoots,
  getFallbackTrashItems,
  ensureFolderMoveIsValid,
  ensureDocumentMoveIsValid,
...
export const cloneWorkspaceSeed = (): WorkspaceSeed => JSON.parse(JSON.stringify(workspaceSeed));
  ...
  return {
    ...
    folders: {
      ...
      moveToSpace: async (id, targetSpaceId) => {
        const currentFolder = seed.folders.find((folder) => folder.id === id && !folder.deletedAt);

        if (!currentFolder || !seed.spaces.some((space) => space.id === targetSpaceId)) {
          throw new Error('Folder move target is invalid.');
        }

        const packageIds = collectTreePackageIds(seed.folders, seed.documents, 'folder', id);
        const folderIdSet = new Set(packageIds.folderIds);
        const documentIdSet = new Set(packageIds.documentIds);
        ...
        rebuildBacklinksForSpace(currentFolder.spaceId);
        rebuildBacklinksForSpace(targetSpaceId);
      },
    ...
    documents: {
      ...
      moveToSpace: async (id, targetSpaceId) => {
        ...
        if (!currentDocument || !seed.spaces.some((space) => space.id === targetSpaceId)) {
          throw new Error('Document move target is invalid.');
        }

        const packageIds = collectTreePackageIds(seed.folders, seed.documents, 'document', id);
        const folderIdSet = new Set(packageIds.folderIds);
        const documentIdSet = new Set(packageIds.documentIds);
        const now = new Date().toISOString();
        const updatedAtLabel = new Date(now).toLocaleDateString('zh-CN');
    ...
    search: {
      query: async (spaceId, query) => {
        ...
        const blockHits: Array<WorkspaceSearchResultRecord & { score: number }> = getActiveDocuments(seed.documents, spaceId)
          .flatMap((document) =>
            extractSearchableBlocks(document.contentJson).map((block) => ({
              ...
              title: document.title,
              preview: buildSearchPreview(document.title, block.searchableText, trimmedQuery),
              spaceId: document.spaceId,
              documentId: document.id,
              blockId: block.blockId,
              fallbackText: block.searchableText,
              ...(document.folderId ? { folderId: document.folderId } : {}),
              score: scoreSearchResult(document.title, block.searchableText, trimmedQuery) + 1,
              searchableText: `${document.title} ${block.searchableText}`.trim(),
            }))
          )
          .filter((record) => includesAllTokens(record.searchableText, tokens))
          .map(({ searchableText: _searchableText, ...record }) => record);
        ...
        return [...blockHits, ...documentHits, ...quickNoteHits].sort((left, right) => {
    ...
    maintenance: {
      ...
      inspectDocumentContentHealth: async (): Promise<DataToolActionResult> => ({
        success: true,
        message: '浏览器 Mock 环境不检查真实文稿格式；仅桌面端 SQLite 数据库会返回旧格式与示例文稿。',
        details: [],
      }),
