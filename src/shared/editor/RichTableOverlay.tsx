...
export const RichTableOverlay = ({
  ...
  const floatingControlsPortal = createPortal(
    <div ref={floatingControlsPortalRef} className="rt-floating-controls-portal" contentEditable={false}>
      {overlayModel.shouldShowTableGrips && overlayModel.colTopAddButtonPos ? (
        <button
          type="button"
          className="rt-col-top-add-button"
          style={{
            top: `${overlayModel.colTopAddButtonPos.top}px`,
            left: `${overlayModel.colTopAddButtonPos.left}px`,
          }}
          contentEditable={false}
          onMouseDown={(e) => e.preventDefault()}
          onMouseEnter={() => actions.setIsEdgeHandleHovered(true)}
          onMouseLeave={() => actions.setIsEdgeHandleHovered(false)}
          onClick={() => commandApi.runAddTableAction(() => commandApi.chainTableFocus()?.addColumnAfter().run() ?? false, '请先定位到单元格')}
          title="当前列右侧添加列"
        >
          <Plus size={12} />
        </button>
